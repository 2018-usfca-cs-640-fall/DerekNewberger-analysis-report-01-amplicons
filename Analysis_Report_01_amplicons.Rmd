...
```{r sample-source-sequence-abundance-barplot}
...
**Figure 1**: Abundance of sequences within the sample_source(s). We see how the abundance of sequences on the each key are not all even. This sparked interest on the difference of abundance between the U and Q keys. From here a focus on the hand or finger used for each key will be compared. 

```{r comparing-abundance-for-q-and-u-keys-barplot}
...
**Figure 5**: Abundance of sequences within the sample_source of the Gkey and Hkey. As shown, the Gkey as on the left side has a higher abundance. The index fingers also show a higher overall abundance than the previous graphs.

```{r comparing-abundance-for-e-and-o-keys-barplot}
...
**Figure 6**: Abundance of sequences within the sample_source of the Ekey and Okey. The Ekey and the Okey were selected for they are the most frequently used key on their respective side. Both the use and the abundance of the Ekey are greater than the Okey. Only one of our graphs show that a key on the right side had a greater abundance.

```{r comparing-abundance-between-left-and-right-hand-keys-boxplot}
...
```{r comparing-mean-abundance-between-handed-keys-barplot}
